...
def create_call(db: Session, payload: schemas.WaiterCallCreate) -> tuple[WaiterCall, list[Diffusion]]:
    ...
    return call, diffusions


def list_pending_calls(db: Session, restaurant_id: int) -> list[WaiterCall]:
...
def resolve_call(db: Session, call_id: int, staff: Staff) -> tuple[WaiterCall, list[Diffusion]]:
    diffusions: list[Diffusion] = []
