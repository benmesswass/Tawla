...
def _setup_restaurant(client):
    restaurant = client.post(
        "/api/v1/restaurants", json={"name": "Café du Coin", "slug": "cafe-du-coin"}
    ).json()
...
def test_restaurant_starts_with_cafe_mode_disabled(client):
    restaurant = client.post("/api/v1/restaurants", json={"name": "Resto Test", "slug": "resto-cafe-default"}).json()
...
def test_cafe_mode_is_isolated_per_restaurant(client):
    ...
    restaurant_b = client.post(
        "/api/v1/restaurants", json={"name": "Autre café", "slug": "autre-cafe"}
    ).json()
