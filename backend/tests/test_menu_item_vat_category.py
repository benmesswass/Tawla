...
def test_menu_item_can_be_created_with_an_explicit_vat_category(client):
...
def test_menu_item_rejects_an_unknown_vat_category(client):
...
def test_menu_item_update_can_change_the_vat_category(client):
...
def test_placing_an_order_copies_the_menu_items_current_vat_category(client, db_session):
...
def test_reclassifying_a_menu_item_never_changes_an_already_placed_order(client, db_session):
