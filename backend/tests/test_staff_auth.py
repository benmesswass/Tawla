...
def test_login_success(client):
    restaurant = create_restaurant(name="Café Auth", slug="cafe-auth")
...
def test_login_wrong_password_is_rejected(client):
    restaurant = create_restaurant(name="Café Auth2", slug="cafe-auth2")
    staff = create_staff(restaurant.id, password=_PASSWORD)
...
def test_me_returns_current_staff(client):
    restaurant = create_restaurant(name="Café Auth3", slug="cafe-auth3")
...
def test_protected_route_requires_token(client):
    restaurant = create_restaurant(name="Café Auth4", slug="cafe-auth4")
...
def test_protected_route_rejects_garbage_token(client):
    restaurant = create_restaurant(name="Café Auth5", slug="cafe-auth5")
...
def test_manager_only_route_rejects_waiter(client):
    restaurant = create_restaurant(name="Café Auth6", slug="cafe-auth6")
