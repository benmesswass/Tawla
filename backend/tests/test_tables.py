...
def _setup_restaurant(client):
    ...
    restaurant = client.post("/api/v1/restaurants", json={"name": "Café Zones", "slug": slug}).json()
...
def test_list_tables_is_isolated_per_restaurant(client):
    ...
    restaurant_b = client.post("/api/v1/restaurants", json={"name": "Autre Café", "slug": "autre-cafe"}).json()
