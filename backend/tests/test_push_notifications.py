...
def _setup_restaurant_with_item(client):
    restaurant = client.post("/api/v1/restaurants", json={"name": "Café Push", "slug": "cafe-push"}).json()
...
def test_client_can_save_push_subscription(client):
    ...
    order_id = _order_to_ready(client, restaurant, table, item, headers)

    res = client.post(f"/api/v1/orders/{order_id}/push-subscription", json=_FAKE_SUBSCRIPTION)
...
def test_marking_ready_sends_push_when_subscribed_and_configured(client):
    restaurant, table, item, headers = _setup_restaurant_with_item(client)
    order_id = _order_to_ready(client, restaurant, table, item, headers)
    client.post(f"/api/v1/orders/{order_id}/push-subscription", json=_FAKE_SUBSCRIPTION)
    ...
    with patch("app.core.push.settings") as mock_settings, patch("app.core.push.webpush") as mock_webpush:
        ...
        res = client.post(f"/api/v1/orders/{order_id}/mark-ready", headers=headers)
        assert res.status_code == 200
