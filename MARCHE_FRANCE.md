...
- [x] **A6 — Allergènes structurés** (14 codes INCO, règlement UE 1169/2011) : `MenuItem.allergen_codes` (chaîne de codes séparés par une virgule, validée à la saisie contre les 14 valeurs — comme `vat_category`/A4), migration. **Coexiste avec le texte libre `allergens`, ne le remplace pas** — même décision que le volet « marqueurs » de [PR #87](https://github.com/benmesswass/Tawla/pull/87) (vocabulaire de régimes libre par restaurant) : un remplacement destructif aurait cassé les fiches déjà saisies en Tunisie, qui n'a aucune obligation des 14 codes UE. `is_halal` par défaut à `false` en France (nouveau champ `Market.default_halal`, jamais un `current_market.code == "fr"` en dur) — corrigé aux TROIS endroits qui le codaient en dur (schéma de création, colonne SQLAlchemy, import CSV) ([PR #150](https://github.com/benmesswass/Tawla/pull/150))
- [x] **UI manager pour `allergen_codes`** — chips à bascule (14 codes) sur les deux marchés, composant partagé avec `vat_category` (A4) — [PR #155](https://github.com/benmesswass/Tawla/pull/155)
...
- [x] **UI manager pour `vat_category`** — select gaté France (`currentMarket.vatRates`), taux affichés dynamiquement (jamais en dur) — [PR #155](https://github.com/benmesswass/Tawla/pull/155)
...
- [x] Montant revérifié côté serveur avant règlement (paiement carte du CLIENT, Connect — pas l'abonnement Tawla ci-dessus) — **flux complet vérifié en conditions réelles le 2026-09-02** : commande créée → `pay/card` → vraie session Stripe Checkout → paiement par carte de test (4242...) → `settle_card_payment` réinterroge Stripe (pas de confiance aveugle au retour client) → commande marquée payée en base (`payment_status=paid`, `paid_at` renseigné). Jusqu'ici réglée UNIQUEMENT par le filet de sécurité `/pay/card/check` (retour client) — pas encore par un webhook Stripe réel
- [x] **Webhook Connect** `POST /api/v1/orders/stripe-card-webhook` (`orders/router.py::order_card_payment_stripe_webhook`), signature vérifiée via `STRIPE_CONNECT_WEBHOOK_SECRET` — DISTINCT de `STRIPE_WEBHOOK_SECRET` de l'abonnement ci-dessus, Stripe attribue un secret par endpoint déclaré. `order_id` retrouvé dans `metadata` de la session Checkout (jamais dans l'URL, contrairement au webhook Konnect qui doit signer son URL lui-même). Route unique, jamais scopée par `{order_id}` — même forme que `stripe-subscription-webhook`. Referme le trou du point précédent : un client qui ferme l'onglet juste après paiement ne laisse plus sa commande `pending` pour toujours ([PR #144](https://github.com/benmesswass/Tawla/pull/144))
- [ ] **Déclarer l'endpoint côté tableau de bord Stripe** 🧑 (Connect > Webhooks, événement `checkout.session.completed`) et poser `STRIPE_CONNECT_WEBHOOK_SECRET` en production — le code est testé (évènements synthétiques, monkeypatch, aucun appel réseau réel), la livraison réelle Stripe → serveur reste à vérifier une fois l'endpoint déclaré, même réserve que le webhook d'abonnement ci-dessus
