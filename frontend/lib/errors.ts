...
const MESSAGES: Record<string, (ctx: Record<string, unknown>) => string> = {
  ...
  ORDER_NOT_CONFIRMED: () =>
    "Cette commande n'a pas encore été confirmée par le serveur. Réessayez dans un instant.",
  CANNOT_CANCEL_PAID_ORDER: () => "Cette commande est déjà payée, elle ne peut plus être annulée.",
...
const AR_MESSAGES: Record<string, (ctx: Record<string, unknown>) => string> = {
  ...
  ORDER_NOT_CONFIRMED: () => "الجرسون ما تأكدش بعد من هاذي الطلبية. عاود جرب من بعد شوية.",
  RATE_LIMITED: () => "طلبت برشة مرات. عاود جرب من بعد شوية.",
};
