...
- [ ] **Activer un vrai moyen de paiement avant le premier palier réellement encaissé** 🧑 — `PAYMENT_MODE=konnect` reste dormant faute de clés réelles (`KONNECT_API_KEY`/`KONNECT_RECEIVER_WALLET_ID`, `backend/.env.example:47-51`) : tout paiement de palier reste en mode démonstration tant que ce n'est pas fait. Décision à prendre par Wassim, aucune des deux options n'est codable en autonomie (KYC, RIB, décision commerciale) : ouvrir un compte Konnect réel (passerelle déjà câblée, sandbox gratuit et inscription en self-service, coût chiffré en `AUDIT_COUTS_PRODUCTION.md` §4.5 — 1,3 % cartes TN/e-dinar, 2,9 % cartes internationales, 2 DT/virement, pas d'abonnement) ou évaluer **ClicToPay** (SMT — Société Monétique Tunisie, distribué par les banques tunisiennes : `Clic to Pay` chez STB, `UIBePAY` chez UIB, etc.) si Konnect ne convient pas le moment venu — adhésion **bancaire**, pas en self-service : nécessite un contrat monétique de vente à distance négocié avec une banque partenaire, commission fixée au cas par cas (non publiée), aucune intégration existante côté Tawla contrairement à Konnect
